// synthetic_scalability.cu - Standalone script for synthetic dataset only
// Compile: nvcc -O2 -std=c++17 -arch=sm_75 synthetic_scalability.cu -o synthetic_scale
// Run: ./synthetic_scale
...
int main() {
    std::cout << "=============================================================\n";
    std::cout << "   SYNTHETIC SCALABILITY - M3\n";
    std::cout << "=============================================================\n";
    
    try {
        // Load synthetic dataset
        Dataset data = load_csv("synthetic_1000k_200f.csv", true);
        
        const int n_samples = data.features.size();
        const int n_features = data.features[0].size();
        ...
        ForestConfig cfg;
        std::vector<double> fractions = {0.10, 0.25, 0.50, 0.75, 1.00};
        
        // Clear old file
        std::remove("benchmark_scalability_synthetic_only_m3.csv");
        
        std::cout << "\n[Running Sequential variant...]\n";
        auto scale_seq = run_scalability_sequential("Synthetic_1M_200f", data, cfg, fractions);
        save_scalability_csv("benchmark_scalability_synthetic_only_m3.csv", scale_seq);
        
        std::cout << "\n[Running Streams variant...]\n";
        auto scale_str = run_scalability_streams("Synthetic_1M_200f", data, cfg, fractions);
        save_scalability_csv("benchmark_scalability_synthetic_only_m3.csv", scale_str);
        
        std::cout << "\n[Running Mutex variant...]\n";
        auto scale_mtx = run_scalability_mutex("Synthetic_1M_200f", data, cfg, fractions);
        save_scalability_csv("benchmark_scalability_synthetic_only_m3.csv", scale_mtx);
        
        std::cout << "\n✅ Complete! Results saved to benchmark_scalability_synthetic_only_m3.csv\n";
        
        // Print summary
        std::cout << "\n=== SUMMARY ===\n";
        std::cout << "Fraction  Sequential(sec)  Streams(sec)  Mutex(sec)\n";
        for (size_t i = 0; i < fractions.size(); i++) {
            std::cout << std::fixed << std::setprecision(2)
                      << fractions[i] << "       "
                      << scale_seq[i].wall_clock_ms / 1000.0 << "           "
                      << scale_str[i].wall_clock_ms / 1000.0 << "           "
                      << scale_mtx[i].wall_clock_ms / 1000.0 << "\n";
        }
